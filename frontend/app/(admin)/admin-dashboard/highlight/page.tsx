...
export default function NewsUpdate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("highlight_news", form.highlight_news);
      ...
      setForm({ title: "", highlight_news: "", image: null });
      await loadNews();
      router.refresh();
    } catch (error) {
  ...
  const deleteNews = async (id: number) => {
    ...
    try {
      const csrfToken = await fetchCsrfToken(API_URL);
      const response = await fetch(`${API_URL}/api/highlight/${id}`, {
        method: "DELETE",
        credentials: "include",
        headers: authHeaders(csrfToken),
      });
  ...
  const handleEditSubmit = async () => {
    ...
    formData.append("highlight_news", editItem.highlight_news);
    ...
    try {
      const csrfToken = await fetchCsrfToken(API_URL);
      const response = await fetch(`${API_URL}/api/highlight/${editItem.id}/update`, {
        method: "POST",
        credentials: "include",
        headers: authHeaders(csrfToken),
        body: formData,
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 p-8">
        ...
        >
          ...
          <label className="block mb-2 text-sm font-medium">Highlight News Title</label>
          <input
            type="text"
            name="title"
            value={form.title}
          ...
          <label className="block mt-4 mb-2 text-sm font-medium">Highlight News Description</label>
          <textarea
            name="highlight_news"
            value={form.highlight_news}
            onChange={handleChange}
            className="w-full p-2 border rounded"
            rows={4}
            required
        ...
        <table className="w-full bg-white shadow-md rounded-lg overflow-hidden">
          <thead className="bg-gray-200">
            <tr>
              ...
              <th className="p-3 text-left">Highlight Title</th>
          ...
          <tbody>
            {currentItems.map((item) => (
              ...
              <tr key={item.id} className="border-t">
                ...
                <td className="p-3">{item.highlight_news}</td>
        ...
        {editItem && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
              ...
              <textarea
                value={editItem.highlight_news}
                onChange={(e) =>
                  setEditItem({ ...editItem, highlight_news: e.target.value })
                }
                className="w-full p-2 border rounded"
                rows={4}
              />
