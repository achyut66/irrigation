...
export default function NewsUpdate() {
  ...
  const loadNews = async () => {
    try {
      const response = await fetch(`${API_URL}/api/karyasamiti`, {
        credentials: "include",
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    //   formData.append("news", form.news);
      ...
      setForm({ full_name: "",designation: "",address:"",mobile_no:"",email:"", image: null });
      await loadNews();
      router.refresh();
    } catch (error) {
  ...
  const deleteNews = async (id: number) => {
    ...
    try {
      const csrfToken = await fetchCsrfToken(API_URL);
      const response = await fetch(`${API_URL}/api/karyasamiti/${id}`, {
        method: "DELETE",
        credentials: "include",
        headers: authHeaders(csrfToken),
      });
      ...
      if (!response.ok || !payload?.status) {
        alert(payload?.message || "Unable to delete karyasamiti. Please try again.");
        return;
      }
      ...
      alert("Unable to delete this karyasamiti right now.");
    }
  };
  ...
  const handleEditSubmit = async () => {
    ...
    formData.append("designation", editItem.designation);
    ...
    try {
      const csrfToken = await fetchCsrfToken(API_URL);
      const response = await fetch(`${API_URL}/api/karyasamiti/${editItem.id}/update`, {
        method: "POST",
        credentials: "include",
        headers: authHeaders(csrfToken),
        body: formData,
      ...
      <div className="flex-1 p-8">
        ...
        >
          ...
          <input
            ...
            required
          />
          <label className="block mt-4 mb-2 text-sm font-medium">Designation</label>
          <input
            type="text"
            name="designation"
            ...
            required
          />
          <label className="block mt-4 mb-2 text-sm font-medium">Address</label>
          <input
            type="text"
            name="address"
            value={form.address}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
              ...
              <input
                ...
                className="w-full p-2 border rounded"
              />
              <label className="block mt-4 mb-2">Designation</label>
              <input
                type="text"
