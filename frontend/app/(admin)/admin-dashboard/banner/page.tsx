...
export default function NewsUpdate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("banner_desc", form.banner_desc);
    //   formData.append("news", form.news);
      ...
      setForm({ banner_desc: "", image: null });
      await loadNews();
      router.refresh();
    } catch (error) {
  ...
  const handleEditSubmit = async () => {
    ...
    formData.append("banner_desc", editItem.banner_desc);
    ...
    try {
      const csrfToken = await fetchCsrfToken(API_URL);
      const response = await fetch(`${API_URL}/api/banner/${editItem.id}/update`, {
        method: "POST",
        credentials: "include",
        headers: authHeaders(csrfToken),
        body: formData,
  ...
  return (
    <main className="ml-60 w-full h-screen max-w-7xl p-6 text-gray-600 bg-gray-100">
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 p-8">
        ...
        <h1 className="text-3xl font-bold mb-6">Banner Details</h1>
        ...
        >
          ...
          <label className="block mt-4 mb-2 text-sm font-medium">Banner Description</label>
          <textarea
            name="banner_desc"
          ...
          <label className="block mt-4 mb-2 text-sm font-medium">Banner Image</label>
        ...
        <table className="w-full bg-white shadow-md rounded-lg overflow-hidden">
          <thead className="bg-gray-200">
            <tr>
              ...
              <th className="p-3 text-left">Banner Description</th>
          ...
          <tbody>
            {currentItems.map((item) => (
              <tr key={item.id} className="border-t">
                ...
                <td className="p-3">{item.banner_desc}</td>
        ...
        {editItem && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
              ...
              <label className="block mt-4 mb-2">Banner Description</label>
              <textarea
                value={editItem.banner_desc}
                onChange={(e) =>
                  setEditItem({ ...editItem, banner_desc: e.target.value })
                }
                className="w-full p-2 border rounded"
                rows={4}
              />
