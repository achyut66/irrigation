...
export default function NewsUpdate() {
  ...
  const loadNews = async () => {
    try {
      ...
      if (!response.ok || !payload?.status) {
        console.error("Unable to load plan details:", payload);
        setNewsList([]);
        return;
      }
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const csrfToken = await fetchCsrfToken(API_URL);
      const response = await fetch(`${API_URL}/api/plan/${editItem.id}/update`, {
        method: "POST",
        credentials: "include",
        headers: authHeaders(csrfToken),
        body: formData,
  ...
  return (
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 p-8">
        ...
        <h1 className="text-3xl font-bold mb-6">Plan & Program Details</h1>
        ...
        {editItem && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
              ...
              <h2 className="text-xl font-semibold mb-4">Edit Plan Program</h2>              
